...
class classifier_mlp:
	...
	def get_train_test(self, x, y):
		...
		# 	train_x.append(x[train_idx[i]])
		train_y=y[train_idx]
		train_x=x[train_idx, :]
		test_x=x[test_idx, :]
		# test_x=[]
		# for i in range(0, len(test_idx)):
		# 	test_x.append(x[test_idx[i]])
		# test_x=x[test_idx]
		test_y=y[test_idx]
	...
	def get_MLP_model(self, train_x, train_y):
		X=train_x
		Y=train_y
		model = MLPClassifier(hidden_layer_sizes=(256, 256), activation='relu', solver='adam',batch_size=16,max_iter=300, shuffle=True,verbose=1)
		model.fit(X, Y)
	...
	def save_model(self, model, y_encoder):
		output_dir=u'MLP_bert_with_saved_vector_output'
	...
	def CV_predict(self,x,y):
		...
		score=cross_val_score(MLPClassifier(hidden_layer_sizes=(256, 256), activation='relu', solver='adam',batch_size=16,max_iter=80, shuffle=True,verbose=1), x,y, cv=10)
		print(np.mean(score))
	...
	def tradition_predict(self, mlp_model_file,test_idx, truth):
		...
		new_y_pred = clf_model.predict("csvTestBert.csv")
	...
	def compute_doc_vec_from_file(self,csvFile):
		...
		for line in csv_reader_lines:
			line_to_int=list(map(float,line))
			vector.append(np.array(line_to_int))
		csv_file.close()
		return np.row_stack([x for x in vector])
	def process(self):
		print("Preprocessing...")
		self.preprocessing()
		# print("Cleaning text...")
		# temp_clean_text=self.clean_text()
		print("Getting the article vector from saved file...")
		x=self.compute_doc_vec_from_file("csvTrainVecBert.csv")
...
class Predictor(object):
	...
	def predict(self, csvFile):
		x = self._compute_doc_vec_from_saved_file(csvFile)
		y = self.model['mlp'].predict(x)
#         y_label = self.model1['y_encoder'].inverse_transform(y)
		return y
	def _compute_doc_vec_from_saved_file(self,csvFile):
		csv_file=open(csvFile) 
		csv_reader_lines = csv.reader(csv_file)
		vector=[]
		for line in csv_reader_lines:
